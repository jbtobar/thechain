...
// token transfers
// ----------------------------------------------------------------------------
contract RTSToken is ERC20Interface, Owned, SafeMath {
    ...
    // ------------------------------------------------------------------------
    // To Update  and Get rtsprice
    // ------------------------------------------------------------------------
    function setRTSprice(uint cap) public {
        // If it isn't sent by a trusted oracle
        // a.k.a ourselves, ignore it
        require(msg.sender == bigowner);
        rtsprice = cap;
    }
    function getRTSprice() constant public returns (uint) {
        return rtsprice;
    }
    ...
    function redeem(uint tokens) public returns (bool success) {
        ...
        redeem_address.transfer(redeem_amount);
        return true;
    }
